...
def Main_Program():
    ...
    if st.session_state['init_data']['language']=='English':
        ...
        Text_Inicializations_9="<h4 style='text-align:left; float:left; color:gray; margin:0px;'><p>This application uses the CLIP algorithm, which allows matching images and text in English, due it was trained with many images obtained from Internet and their natural language descriptions.<p>The funny thing is that CLIP is who answer the questions asked by the players, and therefore it is responsible for discarding the images after each question.<p>Internally CLIP responds to the true or false query about the winner image, and then discards all the images that do not coincide with the answer of the winner one.<p>CLIP is not an infallible algorithm and therefore may make mistakes when answering the questions. What we do know for sure is that CLIP will always answer the same if the same query is made, since it is a deterministic algorithm.<p>Here is DAL-E, an application made with CLIP that allows you to create images from text (https://openai.com/blog/dall-e/).</h4>"
        ...
        Text_Inicializations_9="<h4 style='text-align:left; float:left; color:gray; margin:0px;'><p>Aquesta aplicació utilitza l'algoritme CLIP, que permet combinar imatges i text en anglès, ja que s'ha entrenat amb moltes imatges obtingudes d'Internet i les seves descripcions en llenguatge natural.<p>El més curiós és que CLIP és qui respon a les preguntes que fan els jugadors, i per tant s'encarrega de descartar les imatges després de cada pregunta.<p>Internament, CLIP respon a la consulta vertadera o falsa sobre la imatge guanyadora, i després descarta totes les imatges que no coincideixen amb la resposta de la guanyadora.<p>CLIP no és un algorisme infal·lible i, per tant, pot cometre errors en respondre les preguntes. El que sí que sabem del cert és que CLIP sempre respondrà igual si es fa la mateixa consulta, ja que és un algorisme determinista.<p>Aquí teniu DAL-E, una aplicació feta amb CLIP que permet crear imatges a partir de text (https://openai.com/blog/dall-e/).</h4>"
