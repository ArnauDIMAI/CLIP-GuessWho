    ...
    if num_rows>1:
        for i in range(num_rows):
            for j in range(num_cols):
                if current_index<n_images2:
                    ...
                    axs[i,j].axes.axes.set_xlabel(image_names2[current_index], fontsize=np.int(15))
        ...
        for j in range(num_cols):
            if current_index<n_images2:
                ...
                axs[j].axes.axes.set_xlabel(image_names2[current_index], fontsize=np.int(15))
                ...
                axs[j].spines['bottom'].set_linewidth(3)
                axs[j].spines['top'].set_linewidth(3)
                axs[j].spines['left'].set_linewidth(3)
...
def Load_Images_randomly(num_rows,num_cols):
    ...
    zip_file = zipfile.ZipFile('.\img_celeba_20.zip', 'r')
    for current_name in zip_file.namelist():
        image_files.append(np.array(Image.open(io.BytesIO(zip_file.read(current_name)))))
        image_names.append(current_name[:-4])
   ...
   # Iterate over the file names
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            ...
            axs[i,j].axes.axes.set_xlabel(image_names[current_index], fontsize=np.int(10))
            axs[i,j].axes.xaxis.set_ticks([])
            axs[i,j].axes.xaxis.set_ticklabels([])
            axs[i,j].axes.yaxis.set_visible(False)
            axs[i,j].spines['bottom'].set_color('black')
...
def Token_process_prompt(clip_tokens):
...
def Reload_data():
    ...
    current_prompts=['A picture of a man','A picture of a woman']
    n_tokens,clip_tokens,clip_device,clip_model, clip_transform, clip_text = Token_process_prompt(current_prompts)
    ...
    Data_Init['init_data'][0]['current_prompts']=current_prompts
    ...
    Data_Init['init_data'][0]['user_input_prompts1']='A picture of a man'
    Data_Init['init_data'][0]['user_input_prompts2']='A picture of a woman'
    Data_Init['init_data'][0]['prompts_list']=['A picture of a man', 'A picture of a woman', 'A picture of an attractive person', 'A picture of a young person', 
...
def load_data():
    ...
    Saved_data={
        ...
        'user_input_prompts1':'A picture of a man',
        'user_input_prompts2':'A picture of a woman',
        'prompts_list':['A picture of a man', 'A picture of a woman', 'A picture of an attractive person', 'A picture of a young person', 
            'A picture of a person with receding hairline', 'A picture of a chubby person ', 'A picture of a person who is smiling', 'A picture of a bald person',
            'A picture of a person with black hair', 'A picture of a person with brown hair', 'A picture of a person with blond hair', 'A picture of a person with red hair', 
            'A picture of a person with gray hair', 'A picture of a person with straight hair', 'A picture of a person with wavy hair', 
            'A picture of a person who does not wear a beard', 'A picture of a person with mustache', 'A picture of a person with sideburns', 
...
Feature_Options=['Ask a Question', 'Create your own prompt', 'Create your own 2 prompts','Select a Winner']
...
if Reset_App:
    ...
    if (Data_Init['init_data'][0]['start_game']) and (Data_Init['init_data'][0]['finished_game']==0):
        ...
        if Use_Images_Selected:
            st.markdown("<h2 style='text-align:left; float:left; color:black; margin:0px;'>3. Select a type of Prompt to play.</h2>", unsafe_allow_html=True)
        else:
            st.markdown("<h2 style='text-align:left; float:left; color:black; margin:0px;'>1. Select a type of Prompt to play.</h2>", unsafe_allow_html=True)

        Selected_Feature=st.selectbox('Ask a question from a list, create your prompt or select a winner:', Feature_Options, 
        ...
        ## Select prompt - elements to show (questions)
        if Data_Init['init_data'][0]['selected_feature']=='Ask a Question':
            ...
            Use_Prompt = st.button('USE THIS QUESTION', key='Use_Prompt')

            if Data_Init['init_data'][0]['selected_question']=='Are you bald?' and not Data_Init['init_data'][0]['show_results']:
                Data_Init['init_data'][0]['current_prompts']=['A picture of a person','A picture of a man','A picture of a woman',
                ...
                Data_Init['init_data'][0]['current_prompts']=['A picture of a person who is black-haired',
                ...
                Data_Init['init_data'][0]['current_prompts']=['A picture of a person who is tawny-haired',
                ...
                Data_Init['init_data'][0]['current_prompts']=['A picture of a person who is blond-haired',
                                                            'A picture of a person who is tawny-haired',
                                                            'A picture of a person who is black-haired',
                                                            'A picture of a person who is gray-haired',
                                                            'A picture of a person who is red-haired',
                ...
                Data_Init['init_data'][0]['current_prompts']=['A picture of a person who is red-haired',
                                                            'A picture of a person who is tawny-haired',
                                                            'A picture of a person who is blond-haired',
                                                            'A picture of a person who is gray-haired',
                                                            'A picture of a person who is black-haired',
                ...
                Data_Init['init_data'][0]['current_prompts']=['A picture of a person who is gray-haired',
                                                            'A picture of a person who is tawny-haired',
                                                            'A picture of a person who is blond-haired',
                                                            'A picture of a person who is black-haired',
                                                            'A picture of a person who is red-haired',
                ...
                Data_Init['init_data'][0]['current_prompts']=['A picture of a man','A picture of a woman']
                Data_Init['init_data'][0]['token_type']=0
                Data_Init['init_data'][0]['function_predict']=Predict_0_vs_1    
                
            elif Data_Init['init_data'][0]['selected_question']=='Are you a woman?' and not Data_Init['init_data'][0]['show_results']:
                Data_Init['init_data'][0]['current_prompts']=['A picture of a woman','A picture of a man']
                Data_Init['init_data'][0]['token_type']=0
                Data_Init['init_data'][0]['function_predict']=Predict_0_vs_1         
                
            elif Data_Init['init_data'][0]['selected_question']=='Do you have a beard?' and not Data_Init['init_data'][0]['show_results']:
                Data_Init['init_data'][0]['current_prompts']=['A picture of a person with beard','A picture of a person']
                Data_Init['init_data'][0]['token_type']=0
                Data_Init['init_data'][0]['function_predict']=Predict_0_vs_1
                
            elif Data_Init['init_data'][0]['selected_question']=='Are you YOUNG?' and not Data_Init['init_data'][0]['show_results']:
                Data_Init['init_data'][0]['current_prompts']=['A picture of a young person','A picture of an aged person']
                Data_Init['init_data'][0]['token_type']=0
                Data_Init['init_data'][0]['function_predict']=Predict_0_vs_1

            elif  not Data_Init['init_data'][0]['show_results']:
                Data_Init['init_data'][0]['current_prompts']=[Data_Init['init_data'][0]['prompts_list'][Data_Init['init_data'][0]['questions_index']],'A picture of a person']
                Data_Init['init_data'][0]['token_type']=0
                Data_Init['init_data'][0]['function_predict']=Predict_0_vs_1


        ## Select prompt - elements to show (1 user prompt)
        if Data_Init['init_data'][0]['selected_feature']=='Create your own prompt':
            st.markdown("<h3 style='text-align:left; float:left; color:gray; margin-left:0px; margin-right:0px; margin-top:15px; margin-bottom:-10px;'>Write your own prompt and press the button.</h3>", unsafe_allow_html=True)
            User_Input = st.text_input('It is recommended to use a text like: "A picture of a ... person" or "A picture of a person ..." (CLIP will check -> "Your prompt"  vs  "A picture of a person" )', Data_Init['init_data'][0]['user_input'], key='User_Input', help=None)
            ...
            Check_Prompt = st.button('USE MY PROMPT:   '+Data_Init['init_data'][0]['user_input'], key='Check_Prompt')
            Data_Init['init_data'][0]['token_type']=-1
                

        ## Select prompt - elements to show (2 user prompts)
        if Data_Init['init_data'][0]['selected_feature']=='Create your own 2 prompts':
            st.markdown("<h3 style='text-align:left; float:left; color:gray; margin-left:0px; margin-right:0px; margin-top:15px; margin-bottom:-10px;'>Write your own prompts by introducing 2 opposite descriptions.</h3>", unsafe_allow_html=True)
            User_Input_Prompts1 = st.text_input('Write your "True" prompt:', Data_Init['init_data'][0]['user_input_prompts1'],
                                                        key='User_Input_Prompts1', help=None)
            User_Input_Prompts2 = st.text_input('Write your "False" prompt:', Data_Init['init_data'][0]['user_input_prompts2'],
                                                        key='User_Input_Prompts2', help=None)
            Check_Prompts = st.button('USE MY PROMPTS:   '+User_Input_Prompts1+' vs '+User_Input_Prompts2, key='Check_Prompts')
            Data_Init['init_data'][0]['token_type']=-2
            if Data_Init['init_data'][0]['user_input_prompts1']!=User_Input_Prompts1 and not Data_Init['init_data'][0]['show_results']:
                Data_Init['init_data'][0]['user_input_prompts1']=User_Input_Prompts1
                Data_Init['init_data'][0]['model_changing']=True
                
            if Data_Init['init_data'][0]['user_input_prompts2']!=User_Input_Prompts2 and not Data_Init['init_data'][0]['show_results']:
                Data_Init['init_data'][0]['user_input_prompts2']=User_Input_Prompts2
                Data_Init['init_data'][0]['model_changing']=True


        ## Select prompt - elements to show (winner selection)
            ...
            if Data_Init['init_data'][0]['selected_feature']=='Ask a Question':
                if Use_Prompt:
                    [ Data_Init['init_data'][0]['n_tokens'],
                      ...
                      Data_Init['init_data'][0]['clip_text'] ]=Token_process_prompt(Data_Init['init_data'][0]['current_prompts'])
                    ...
                    if Data_Init['init_data'][0]['selected_question']=='Are you a MAN?' or Data_Init['init_data'][0]['selected_question']=='Are you a WOMAN?':
                        del Data_Init['init_data'][0]['prompts_list'][0:2]
                        del Data_Init['init_data'][0]['feature_questions'][0:2]
                    else:
                        del Data_Init['init_data'][0]['prompts_list'][Data_Init['init_data'][0]['questions_index']]
            ...
            if Data_Init['init_data'][0]['selected_feature']=='Create your own prompt':
                if Check_Prompt:
                    if Data_Init['init_data'][0]['user_input']!='A picture of a person':
                        Data_Init['init_data'][0]['current_prompts']=['A Picture of a person',Data_Init['init_data'][0]['user_input']]
                        ...
                        st.markdown("<h3 style='text-align:left; float:left; color:red; margin-left:0px; margin-right:0px; margin-top:15px; margin-bottom:-10px;'>Your prompt must be different of 'A picture of a person'.</h3>", unsafe_allow_html=True)

            # User 2 prompts
            if Data_Init['init_data'][0]['selected_feature']=='Create your own 2 prompts':
                if Check_Prompts:
                    # st.sidebar.markdown('(new 2 own prompts:)')
                    Data_Init['init_data'][0]['current_prompts']=[Data_Init['init_data'][0]['user_input_prompts1'],Data_Init['init_data'][0]['user_input_prompts2']]     
                    Data_Init['init_data'][0]['function_predict']=Predict_0_vs_1
                    [ Data_Init['init_data'][0]['n_tokens'],
                      Data_Init['init_data'][0]['clip_tokens'],
                      Data_Init['init_data'][0]['clip_device'],
        ...
        if Data_Init['init_data'][0]['show_results']:
            if not np.sum(Data_Init['init_data'][0]['current_images_discarted']==0)==1:
                ...
                Next_Prompt=st.button('NEXT PROMPT', key='Next_Prompt')
            ...
            if Data_Init['init_data'][0]['token_type']==-2:
                if Data_Init['init_data'][0]['image_current_predictions'][Data_Init['init_data'][0]['current_winner_index']]:
                    st.markdown("<h3 style='text-align:left; float:left; color:blue; margin-left:0px; margin-right:25px; margin-top:0px; margin-bottom:0px;'>The most accurate prompt is:</h3><h3 style='text-align:left; float:left; color:green; margin:0px;'>"+Data_Init['init_data'][0]['user_input_prompts1']+"</h3>", unsafe_allow_html=True)
                else:
                    st.markdown("<h3 style='text-align:left; float:left; color:blue; margin-left:0px; margin-right:25px; margin-top:0px; margin-bottom:0px;'>The most accurate prompt is:</h3><h3 style='text-align:left; float:left; color:green; margin:0px;'>"+Data_Init['init_data'][0]['user_input_prompts2']+"</h3>", unsafe_allow_html=True)
